        ...
        <div class="d-flex justify-content-around flex-wrap">
            @foreach ($role as $permission)
                <div class="mt-4 w-25 text-center">
                    <input type="checkbox" class="i-checks chk-activePermissions {{ $key }}-permission data-{{ $key }}" name="permission[]" data-role="{{ $key }}"" value="{{ $permission->id }}" {{ ($result['permissions']->contains('id', $permission->id)) ? "checked" : "" }} >
